...
from preprocessing_kp import parse_annotation_kp
...
class YOLO(object):
    def __init__(self, backend,
        ...
        if backend == 'ResNet50':
            ...
            raise Exception('Architecture not supported! Only support MobileNet and ResNet50 at the moment!')
...
class keypoints(object):
    def __init__(self, backend,
        ...
        output = Dense(classes, activation='softmax', name='fc' + str(classes), kernel_initializer = glorot_uniform(seed=0))(output)
    ...
    def delta_(self, p1, p2, x, y):
        return math.sqrt((x[p1]-x[p1])**2 + (y[p2]-y[p2])**2)

    def cross_ratio(self, surf, x, y):
        if surf == "left":
            return (self.delta_(1, 3, x, y)/self.delta_(1, 4, x, y)) / (self.delta_(2, 3, x, y)/self.delta_(2, 4, x, y))
        elif surf == "right":
            return (self.delta_(1, 6, x, y)/self.delta_(1, 7, x, y)) / (self.delta_(5, 6, x, y)/self.delta_(5, 7, x, y))
    ...
    def custom_loss(self, groundtruth, pridection):      # ETH custom loss
        ...
        for i in range(0, 14, 2):
            self.X_predicted[i] = pridection[i]
            self.X_groundtruth[i] = groundtruth[i]

        for j in range(1, 14, 2):
            self.Y_predicted[j] = pridection[j]
            self.Y_groundtruth[j] = groundtruth[j]
    ...
    def train(self, train_imgs,     # the list of images to train the model
        ...
        X_train ,Y_train =  parse_annotation_kp()
